...
impl TrState {
    ...
    fn finalize<R: Rng>(self, rng: &mut R) -> TrData {
        let enc = PrivcountEncryptor::new(&self.keys.enc_key, &self.keys.signing_key);
        let u64s = Vec::from_iter(self.counters.into_iter().map(|fe| fe.value()));
...
impl CounterSet {
    pub fn new<R: Rng>(rng: &mut R, counter_ids: &[CtrId], tr_ids: &[TrKeys], k: usize) -> Self {
        let counter_ids = counter_ids.to_vec();
        let n_counters = counter_ids.len();
        let mut tr_states = Vec::from_iter(tr_ids.iter().map(|k| TrState::new(rng, k, n_counters)));
    ...
    pub fn finalize<R: Rng>(mut self, rng: &mut R) -> CounterData {
        ...
        let tr_data = Vec::from_iter(self.tr_states.into_iter().map(|state| state.finalize(rng)));
